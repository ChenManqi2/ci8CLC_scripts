...
DataLink()
{
   ...
   ls ${indir} | grep -f ${sample} - | grep -v "md5" | while read file
   do
...
BamToDedupPara()
{
   indir=$1; outdir=$2; logdir=$3
   [ ! -d "${outdir}" ] && mkdir -p ${outdir}; [ ! -d "${logdir}" ] && mkdir -p ${logdir}
   find ${indir} -name "*psorted.bam" | xargs basename -s ".bam" | parallel -P 3 --gnu "sambamba markdup -r -t 12 ${indir}/{}.bam ${outdir}/{}_dedup.bam > ${logdir}/{}_dedup.log 2>&1"
   find ${outdir} -name "*dedup.bam" | xargs basename -s ".bam" | parallel -P 3 --gnu "sambamba index -t 12 ${outdir}/{}.bam > ${logdir}/{}_index.log 2>&1"
}
# Subread
CountGene()
{
...
CountRepeatUni()
{
   ...
   [ ! -d "${outdir}" ] && mkdir -p ${outdir}; [ ! -d "${logdir}" ] && mkdir -p ${logdir}
   bamlist=$(ls ${indir}/*.bam | tr '\n' ' '); echo ${bamlist}
   featureCounts -F SAF -T 12 -s 0 -a ${saf} -o ${outdir}/all_samples_repeat_count_matrix.txt \
                 ${bamlist} > ${logdir}/all_samples_repeat_count_matrix.log 2>&1
}
# TEtranscripts
TEcountPara()
...
TElocusPara()
{
   ...
   ge_head1k=${outdir}/gene_gtf_head1k.gtf
   find ${indir} -name "*${key}*" | xargs basename -s ".bam" \
      | parallel -P 3 --gnu "TElocus -b ${indir}/{}.bam --GTF ${ge_head1k} --TE ${te_anno} --format BAM --mode multi \
                                     --sortByPos --stranded no --project ${outdir}/{}_tecount > ${logdir}/{}.log 2>&1"
}
...
# - 1st step: Organize the workding directory (required)
...
# - 2nd step: Quality control of raw data (required)
...
# - 3rd step: Trim and filter raw reads (required)
...
# - 5th step: Quantification (required)
...
CountRepeatUni ${wd}/results/star/repeat ${repeat_saf} ${wd}/results/featurecounts/repeat_uni ${wd}/logs/featurecounts/repeat_uni
echo "Finish at $(date)"
